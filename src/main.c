...
void draw(double real_start, double real_end, double imaginary_start,
  ...
  for (int x = 0; x < WINDOW_WIDTH; x++) {
    for (int y = 0; y < WINDOW_HEIGHT; y++) {
      Complex c = {real_start +
                       ((double)x / WINDOW_WIDTH) * (real_end - real_start),
                   imaginary_start + ((double)y / WINDOW_HEIGHT) *
      ...
      SDL_Colour colour = hsv_to_rgb(hue, (double)saturation, (double)value);
...
int main(void) {
  ...
  SDL_Window *window =
      SDL_CreateWindow("mandelbrot_c", SDL_WINDOWPOS_CENTERED,
